...
* Typically contain a lot of points (>1,000).
    + Sensors, Actuators (inputs ouputs)
    + Software point: schedule, trend logs, calculations, processing Virtual points

Read sensors, process things, do stuff to actuators (e.g. fans motors dampers). Sensors are temperature humidity pressure. My sensor def is also metadata inside controller.

----

<div class="centered">

![](fig/bms-components.jpg)
...
Simple example - if high temperature readings are recorded in a room, dampers will open and air handlers will modulate to provide cooler air.

BMS systems are costly to implement and to change. Can often require large amounts of recoding just to tune the BMS's behaviour.

Where is the BMS working well and where is it working badly? Find issues on BMS operation.

Heating valve locked open - lots of cooling to make the room work. When things go wrong can take a lot of time to dig in and fix it up.

The bigger the BMS is the harder it is to find what matters.
...
* Buildings Alive's goal is to
    ...
    + Analyse and present data into useful information.

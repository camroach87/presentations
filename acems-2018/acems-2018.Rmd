...
Consider attribute $h$ for building $j$ changing from false to true. Let $q_{0.5} \left( Y | X \right)$ be the conditional median of $Y$ on $X$. Then,

$$
q_{0.5} \left( Y_{ij} | X_{hij} = 1 \right)
\approx q_{0.5} \left( Y_{ij} | X_{hij} =  0 \right) e^{\beta_h}.
$$


<!-- $$ -->
<!-- \begin{gathered} -->
<!-- q_{0.5} \left( \log Y_{ij} | X_{hij} = 1 \right) - q_{0.5} \left( \log Y_{ij} | X_{hij} = 0 \right) = \beta_h \\ -->
<!-- \Rightarrow q_{0.5} \left( Y_{ij} | X_{hij} = 1 \right) -->
<!-- \approx q_{0.5} \left( Y_{ij} | X_{hij} =  0 \right) e^{\beta_h}. -->
<!-- \end{gathered} -->
<!-- $$ -->

Calculating the median demand using this formula gives our profile plots. The values $e^{\beta_h}$ can be interpreted as the percentage change when $X_{hij}$ changes value from false to true.
...
__TODO:__ Check quantile properties, or show the approximation for log is ok.
